...
import { IDeleteUserFcmTokenConfirmationModalProps } from "./index";
import { toast } from "react-toastify";

export const DeleteUserFcmTokenConfirmationModal: React.FC<IDeleteUserFcmTokenConfirmationModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  config,
  ...
  const handleDelete = async () => {
    try {
      ...
      await onConfirm();
      onClose();
      toast.success(`user fcm token "${config.id}" deleted successfully!`);
    } catch (error) {
    } finally {
      setIsLoading(false);
    }
